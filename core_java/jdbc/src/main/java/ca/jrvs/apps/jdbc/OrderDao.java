...
public class OrderDao extends DataAccessObject<Order> {
    ...
    private static final String GET_ORDER = "SELECT c.first_name, c.last_name, c.email, o.order_id, "
        ...
        try (PreparedStatement statement = this.connection.prepareStatement(GET_ORDER)) {
            ...
            while (rs.next()) {
                if (orderId == 0) {
                    ...
                    orderId = order.getId();
                    order.setOrderCreationDate(new Date(rs.getDate(5).getTime()));
                    order.setOrderTotalDue(rs.getDouble(6));
                    order.setOrderStatus(rs.getString(7));
                    order.setSalesFirstName(rs.getString(8));
                    order.setSalesLastName(rs.getString(9));
                    order.setSalesEmail(rs.getString(10));
                }
